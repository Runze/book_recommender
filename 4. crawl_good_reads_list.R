...
gr_df_s = subset(gr_df, rating >= 4)
gr_df_s = subset(gr_df_s, rank <= quantile(rank, .5))
...
for (i in 1:nrow(gr_df_s)) {
  ...
  if(!is.na(book_ps)) {
    ...
    if('English' %in% lang) {
      genres = tryCatch(html_text(html_nodes(book_ps, '.elementList ')),
                        error = function(cond) return(NA))
      ...
      descs = tryCatch(html_text(html_nodes(book_ps, '#description span')), 
                       error = function(cond) return(NA))
      ...
      gr = data.frame(rbind(gr, c(gr_df_s$title[i], gr_df_s$author[i], genre, desc, img, amazon))) 
    }
  }
}
